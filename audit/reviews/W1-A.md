...
- **R1 is resolved.** `handleContact` saves the number as `+<digits>` exactly as Telegram sent it. Nothing adds a country code any more; `normalizePhone` is no longer imported in `telegram.ts`. Numbers with fewer than 7 or more than 15 digits are rejected. `isAdminPhone` compares digits exactly, so a 9-digit number is saved as `+<9-digit national form>` and is not admin. Test 5 now expects that.
- **No non-admin path left.** An account passes `isAdminPhone` only if the saved number's digits exactly match `998XXXXXXXXX`. The only way to save a number is a contact that meets all of these:
...
- **The real admin's own share still works.** Telegram sends `998XXXXXXXXX` (sometimes with a `+`), which is saved as `+998XXXXXXXXX` and passes the admin check; test "o'zining xalqaro shakldagi admin raqami" covers it. `chat.type` is a required Bot API field, so real private chats always get `"private"`.
...
- **Deploy note still applies:** on prod, confirm that only the admin's own `telegram_id` holds `998XXXXXXXXX` or `<9-digit national form>` in either form.

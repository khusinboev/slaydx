...
- `f48b052` **WP-C (lead)** — `ArticleType.guidance` (12 turga yozish qoidalari, tizim promptida «TYPE RULES»), `ArticleType.judge` (turga bog'liq baholovchi: `describe` — sharh/tizimli sharh/metodik/tahliliy/CARE/qisqa xabar o'z mezon ta'rifi; `skip` — tezis taqqoslash+metodlar, uch qismli metodlar; `JudgeResult.skipped` ballga/bandlarga kirmaydi, `judgeFromReview` tiklaydi), profil `maxPages` → hisobot `pageLimit` (`estimateDocPages`, qisqartirish fix), `tests/article-matrix` (12 tur juft-juft skelet/qoida/reja prompti; baholovchi promptlari; 5 profil render+hisobot); 4 mutatsiya. `a860cfe` konferensiya to'plami tahlili (§2a) → `FILLER_PHRASES` +7.
- `db1a03e` **WP-A (opus, 5 kommit)** — Avto-sayqal: `article/polish.ts` (sof: `planPolish` — qoidalar + baholovchi tavsiyalari → ≤6 fix, `length`/`visuals` sintezi ([fig:]/[tab:] tokeni), Q-2 `needsUserData` (kuchli/kuchsiz+fe'l lug'ati en/uz/ru), bitta nishon = bitta fix; `applyPolish` mapPool 3; `runPolish` → `reviewArticle(judge)` → Q-3 faqat ball OSHSA, baholovchi javobsiz → eski ballar; `userNeeds` → `review.userNeeds`), «Tuzatish» sof qismi `rewriteFix` shu yerga (`article-rewrite.ts` o'ram), engine parserlari `parse.ts` (sharp izolyatsiyasi), `review.ts visualCoverage`; engine 8-bosqich 92→96 (`score < 90`, `remainingMs ≥ 90 s`, aks holda `polish.skipped: budget`; `ARTICLE_POLISH=0`/live `--no-polish`), byudjet +90 s; `POST …/polish` (3/maqola/kun, 20/foydalanuvchi/kun; qabul → op lar+review, rad → faqat review; kredit yo'q) — `rate_limits` purge 2 soat → 25 soat (kunlik oyna yo'qolardi); UDK taklifi `POST /api/article/udk` (`fast`, `parseUdk`, «tekshiring»); UI: «Hammasini tuzatish», «Sizdan kutiladi» (udk/authors/results → forma langarlari), sayqal jurnali, forma UDK «Taklif». Testlar: article-polish 11, article-polish-route 5, ui/article-polish 6, engine +2, panel +3, composer +1; 16 mutatsiya. Topilma: baholovchi sarfi ikki marta sanalardi (o'ram + onUsage).
- `3c1a9f0`→`b5f18e6` **WP-B (opus, 4 kommit)** — 5 yangi sxema turi: `layers` (2–7 qatlam × ≤4 band, ikki tomonlama o'q), `cycle` (3–8 bosqich ellips bo'ylab, yoy o'qlar `<path A>`, markaz, clockwise), `timeline` (3–10 voqea, `when` ostida, yorliq navbatma-navbat, >6 → ikki qator), `matrix` (aynan 4 kvadrant, ixtiyoriy o'qlar — SWOT o'qsiz), `compare` (ikki ustun ≤6 / `rows` bilan 3 ustunli mezon jadvali); `figures/layout-{layers,cycle,timeline,matrix,compare}.ts`, `model.ts` primitivlar (arc/dot/arrowStart/none/indent) + `wrapToWidth` (belgi soni tor harfli so'zlarda erta o'rardi), `svg.ts` arrow-rev marker. `figureSpecFromLlm` 5 tur tekshiruvi + `figureKinds` oq ro'yxati (chart mustaqil); prompt `FIGURE_KIND_HELP` «use for» + tur tavsiyasi (analytical → matrix/compare/flow, review → timeline/layers/tree, methodical → process/cycle, imrad → flow/layers, CARE → timeline) + «Allowed kinds»; reyestr `figureKinds` (16-parametr, json, zond `figures`+`prompt`); `ArticleComposer` «Sxema turlari» chips (Avto + 8, sxema 0 → o'chiq, qoralama). 33 test, 7 mutatsiya, 10 PNG ko'z (cycle yoy uzunligi px, matrix y-o'q alohida ustun, «•» chekinish tspan x — librsvg NBSP'ni yutadi). Ochiq: ruxsatsiz tur → sxema tushib qoladi (retry yo'q); cycle 8 ≈ 124 mm.
- **R (lead)**: jonli `article-oak` sayqal bilan 78 → 92 — LEKIN **Q-2 buzilgan**: baholovchining «statistik testlar, p-qiymatlar, platforma nomini ko'rsating» tavsiyasi bajarilib model Moodle/t-test/ANOVA/stratified randomization ni o'ylab topdi (faktlarda yo'q), baholovchi mukofotladi → `needsUserData` filtri faktlar bo'lsa ham (`unreported`), lug'at +platform/software/randomis/protocol/hyperparameter/t-test/anova (uz/ru), qayta yozish promptida **HONESTY LIMIT** (bo'lim va annotatsiya), baholovchi promptida «tavsiya keltirilmagan tajriba tafsilotini so'ramasin» — qayta jonli: o'ylab topish yo'q, model «platforma nomi, parametrlari va randomizatsiya mezonlari muallif tomonidan ochiqlanmagan» deb yozdi. Keyin Claude 0 fix qaytardi → `criterionFixes` (past mezon → halol ko'rsatma: novelty→kirish, overclaim/chain→xulosa, comparison→muhokama, methods→faqat faktlar bilan) — jonli 81 → 83 qabul, 5 bo'lim; `article-review` 94 (≥ 90, sayqal yo'q; turga bog'liq baholovchi methods 3/3), `article-kinds` (yangi live holat: cycle/matrix/compare majburiy) — uchalasi LLM spetsifikatsiyasidan PNG, ko'z bilan to'g'ri, 69 → 77. Forma: profil bet chegarasi ogohlantirishi (`data-pages-limit`). Chromium smoke (`pw/article18.mjs`): UDK taklifi «621:004.94» + izoh, chips 9 (matrix+compare → dvigatelga yetdi), avto-sayqal jurnali («ballni oshirmadi 79 → 73 — eski matn qoldirildi» — Q-3 ishladi), «Sizdan kutiladi», «Hammasini tuzatish» → 79 → 79, 0 brauzer xatosi. To'liq to'plamlar: unit 1484/1487 (2 tasi `.env.local` fal.ai), ko'ruvchi 143, UI 169, lint/tsc toza.

## 6. Ochiq bandlar

- Claude baholovchi ballari bir xil matnda ±5 tebranadi — sayqal qabul/rad chegarasi (`>`) shovqinga sezgir; kerak bo'lsa `≥ +2` yoki ikki bahoning o'rtachasi.
- Ruxsat etilmagan sxema turi (`figureKinds`) → bo'limda sxema chiqmaydi (qayta so'rov yo'q); `cycle` 8 bosqich ≈ 124 mm.
- «Hammasini tuzatish» 3/maqola/kun — foydalanuvchi ma'lumot qo'shgach (UDK, ORCID) qayta yaratmasdan hisobotni yangilash yo'li yo'q (faqat ko'ruvchida tahrir orqali).
- UDK taklifi LLM bilimidan (to'plamda UDK yo'q) — asosiy sinf darajasida ishonchli, kichik bo'limlar tekshirilishi kerak (izoh shuni aytadi).

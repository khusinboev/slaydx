...
## 7. Deploydan keyingi tuzatishlar (2026-09-11)

**Tarjimon «Fayl» tabi — «content blocked».** Ikki sabab: (1) sayt CSP'sida `frame-src` faqat Telegram domenlariga ruxsat berardi, `'self'` yo'q edi — brauzer sahifaning O'Z PDF'ini iframe'da ochishini to'sardi; (2) `?format=pdf` javobi `Content-Disposition: attachment` bilan kelardi — iframe'da ko'rsatish o'rniga yuklab olishga o'tardi. AUDIT-14 smoke buni ko'rmagan, chunki headless-shell Chromium iframe ichida PDF chizmaydi («iframe: 1» deb hisoblagan). Tuzatish: `frame-src 'self' …`; fayl marshruti uchun alohida CSP `frame-ancestors 'self'` (konfiguratsiya sarlavhalari marshrutnikini USTIDAN yozadi — prodda `thumb` ning o'z CSP'si yo'qolgani shundan); `inline=1` parametri → `inline`; ko'ruvchi iframe va «yangi oynada ochish» havolasi `fileUrl(id, "pdf", { inline: true })`. Testlar: `tests/csp-headers.test.mts` (mutatsiya: `'self'` olib tashlansa qizaradi), `tests/ui/translation-viewer-file.test.mts`. Tekshiruv: to'liq Chromium (`channel: "chromium"`, yangi headless — PDF ko'ruvchisi bor) «Fayl» tabida PDF ko'ruvchisini chizdi, CSP xatolari 0.

**Yaponchaga tarjima qilingan PPTX ko'rinishida «□□□».** PPTX faylning o'zi to'g'ri (`自動車工学`, `内燃機関と電気自動車の違い`), lekin PDF ko'rinishi va eskizni LibreOffice web konteynerida o'giradi, konteynerda esa faqat `font-noto` (lotin/kirill) bor edi — CJK va arab yozuvi uchun glif yo'q. Tuzatish: Dockerfile'ga `font-noto-cjk` va `font-noto-arabic` (+`fc-cache`), ~60 MB. Ochiq band: PPTX'da `<a:ea typeface="Arial">` — PowerPoint o'zi CJK shriftga almashtiradi; xohlasak `pptx.ts` adapteri CJK tillar uchun `ea` ni «Noto Sans CJK» ga qo'yishi mumkin.

Deploy `0a3e35a` (2026-09-11): zaxira `slaydx-20260911072338.sql`, rollback `e9aeba0`. Eslatma: birinchi urinishda SSH sessiyasi build oxirida uzilib skript `up -d` ga yetmadi (image yasalgan, konteynerlar eski) — qayta `nohup bash deploy.sh` bilan; bundan keyin uzoq deployni `nohup` bilan yurgizish kerak. Prod tekshiruvi: konteynerda yapon shriftlari 30, arab 72; yaponcha PPTX PDF'ida `NotoSansCJKhk` embed, 1-slayd to'g'ri (`自動車工学 / 内燃機関と電気自動車の違い`); «Fayl» tabi to'liq Chromium'da PDF ko'ruvchisi bilan ochildi (javob `inline`, CSP `frame-ancestors 'self'`, xato 0); ja/zh/ko/ar tarjimalarining eskiz keshi tozalandi (1 qator), qayta yasalgan eskiz to'g'ri.

**18 til tekshiruvi (2026-09-11).** Prod web konteynerida (`0a3e35a` dan keyin) barcha 18 til uchun maxsus harfli namunaviy DOCX (Times New Roman + Arial) va PPTX (Arial, tarjimon chiqargan fayl kabi) o'girildi: qoraqalpoq (ǵ ń ú ó ı), turkman (ý ž ň ş ä), turk (ş ğ ı İ), qozoq (ә ғ қ ң ө ұ ү һ і), qirg'iz (ң ө ү), tojik (ҳ ҷ ғ қ ӯ ӣ), rus, arab (RTL, shakllangan, arab raqamlari), xitoy, koreys, yapon, nemis/fransuz/ispan/italyan/portugal diakritikalari — hammasi to'g'ri, «□» yo'q. PDF'ga embed bo'lgan shriftlar: LiberationSans (lotin/kirill), NotoSansArabic, NotoSansCJKhk, NotoSansMath. `tests/dockerfile-fonts.test.mts` paketlar ro'yxatini qulflaydi (mutatsiya: `font-noto-cjk` olib tashlansa qizaradi).

**Natija sahifasi oqim rejimi** (`8d1b43f`, deploy: zaxira `slaydx-20260911160149.sql`, rollback `0a3e35a`): tarjima natijasida `ResultView` ildizi `overflow-y-auto`, peshtoq `sticky` emas, `TranslationViewer` ichki scroll qutisiz, iframe `calc(100vh − 4.5rem)`. Chromium (lokal va prod): scroll qilinganda peshtoq 56 → −136 px ga chiqib ketdi, iframe 800 px ekranda 728 px; slayd sahifasi eski qat'iy rejimda (`overflow-hidden`, flow belgisi yo'q). Testlar: `tests/ui/result-flow.test.mts`, `translation-viewer` (ichki scroll qutisi yo'qligi).

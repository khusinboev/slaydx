...
## 5. Deploydan keyingi tuzatishlar (foydalanuvchi sinovi, 2026-09-10)

- **Tarjima: «89 banddan 24 tasi tarjima qilinmadi».** Jadvaldagi Python kod qatorlarini model to'g'ri o'zgarishsiz qaytargan, tekshiruv esa `same` ni xato deb ishni yiqitgan. Endi o'zgarishsiz band qabul qilinadi (qayta so'ralmaydi), ≥80 belgi bo'lsa `unchanged` ogohlantirishi; sabablar worker jurnalida. Shu fayl qayta: 98/98 band, `referat-tk.docx`.
- **Taqqoslash ko'rinishida scroll yo'q edi** — `ResultView` konteyneri `overflow-hidden`; ro'yxat endi o'z `overflow-y-auto` qutisida (`data-translation-scroll`).
- **Eski ↔ yangi shablon to'qnashuvi** (karta eski dizaynda, sayt yangi, fayl eski): vizual har safar joriy reyestrdan olinardi. Endi `AcademicDoc.slideVisual` render vaqtida qadaladi (`buildSlideAcademicDoc`), `buildSlideDeck` unga ustunlik beradi (noma'lum qiymat e'tiborsiz); eski qatorlar `018_slide_visual_pin.sql` bilan `preview.slide.visual` dan to'ldiriladi (`doc_prev` ham). Titul rasmi «saytda yo'q, faylda bor» holati ham shu ildiz (`defense`: `dense` → `formal`).
- **Logotip PPTX'da eniga cho'zilgan** — pptxgenjs `sizing.contain/cover` Node'da rasm o'lchamini bilmaydi va quti nisbatini oladi. `render-pptx` endi baytlardan o'lcham o'qiydi (`imageDims`): `contain` — quti ichida markazda, nisbat saqlanadi; `cover` — rasm nisbati bilan `srcRect` kesish (ko'ruvchi `object-fit` bilan bir xil). Testlar `tests/pptx-image-fit.test.mts` (2 mutatsiya qizil), `tests/slide-visual-pin.test.mts`; ko'z bilan: 4:1 logotip va 4:1 foto PNG'da to'g'ri.

## 6. Ikkinchi sinov (2026-09-10, kechqurun)

- **Yangi dekalarda rasm joyi bo'sh** — `imageBudget` shifti (10 slaydda 8) dekaning oxiridagi bo'lim/yakun slaydlarini rejadan tashqarida qoldirardi (`got = want`, ya'ni provayder emas, REJA). Formalar 2 dan beri oddiy slayd rasmlari bepul stock — shift olib tashlandi: `photoSlot` bergan har slayd rasm oladi (pro bilan bir xil). Testlar yangilandi.
- **Eski faylda logotip cho'zilgan** — saqlangan PPTX yaratilgan paytdagi kod bilan. `scripts/rerender-pptx.mts` (worker konteynerida): joriy renderer + qadalgan vizual bilan qayta yasaydi; media soni kamaysa (aktiv yo'q / vizual qadalmagan eski qator) o'sha faylga tegmaydi.
- **Kartada asl faylning ko'rinishi (hamma tur)** — `GET /api/generations/{id}/thumb`: DOCX/PPTX 1-sahifasi LibreOffice → `pdftoppm` (36 dpi, JPEG q72, ~8–25 KB), aktiv sifatida keshlanadi (`THUMB_ASSET_ID`), brauzer keshi 1 kun; bir vaqtda ≤2 LibreOffice, bir id uchun bitta ish. `FilePreview`: DOCX/PPTX → `<img loading="lazy">`, yuklanguncha/xatoda matn qatorlari; slayd (`SlideCanvas`) va rasm o'zgarmadi. Og'ir fayl kartaga tushmaydi — faqat eskiz. Lokal: sovuq 2.3 s / 7.8 KB, keshdan 0.27 s.

...
- **R3 — Chromium smoke** (dev 3111 + worker, scratch `teacher20.mjs`): `lesson-plan` forma (17 maydon, 4 000) → 1 daq 27 s → hisobot 97 (16 band) → «Tahrirlash» → tahrir `docVersion` 1 → DOCX 12 KB; `test` (24 maydon, 3 000) → 1 daq 48 s → hisobot 96–97 (30 band, «Sizdan kutiladi» 2), avto-sayqal qabul, tahrir, DOCX 297 KB (OMR PNG ichida — birinchi urinishda composer `omr:false` standarti dvigatel `true` bilan mos kelmagani uchun OMR chiqmagan, tuzatildi); «Hammasini tuzatish» 422 `nothing` («tuzatiladigan band yo'q — qolganlari sizning ma'lumotingizni kutmoqda») — kutilgan xulq, brauzer xatosi faqat fetch statusi. pgrep: dev/worker to'xtatildi.
- **Deploy (2026-09-17)** — zaxira (374 MB), rollback `552a46c`, `nohup deploy.sh` → prod `5852285`, konteynerlar sog'lom, health ok. **Prod smoke** (egasi hisobi): eski dars rejasi/xarita/glossariy/keys hujjatlari — API/fayl/sahifa 200, Chromium 0 xato (4/4, legacy yo'li); yangi dars rejasi (`d46bccaa…`, 15 s, 100 ball, DOCX 12 KB) va test (`46aed661…`, 27 s, 100 ball, sayqal, tahrir docVersion 1, DOCX 296 KB — OMR PNG ichida). **Topilma:** prod'da Anthropic hisobi tugagan (`400 credit balance is too low`) → baholovchi zanjir bo'yicha Gemini flash ga tushdi (1,5 s, 100 ball — yumshoqroq); tizim buzilmagan, lekin Claude baholovchi uchun hisobni to'ldirish kerak. «Hammasini tuzatish» 100 ballda 422 `nothing` — kutilgan.

## 6. Ochiq bandlar
- Anthropic hisobi (prod) to'ldirilishi — baholovchi sifati (Gemini flash 100 ball beradi, Claude 87–97).
- O'quv bazasi: 1–4 sinf, informatika, ona tili/adabiyot, chet tillari yo'q; matematika 9, geografiya 11 manbada yo'q; `gen-curriculum --llm` tozalash ishlatilmagan; 121-son buyruq PDF 502 (soat tekshiruvi dastur ichidagi yillik soatga).
- Eski yozuvchi yo'li (`write-specials.ts`, `TEACHER_ENGINE=0`) — keyingi sprintda o'chirish (testlari bilan).
- `TEACHER_PAGES` bet chegaralari keng (WP-F) — jonli 7 holat bo'yicha toraytirish.
- Tahrirda son maydonlari (grade/duration/soat) rad etiladi — formada; test B variant tahriri A da ham ko'rinadi (bitta savol bazasi) — ko'ruvchida izoh.
- `keys`/`test` nasr bo'limlarida `blockRemove` model ro'yxatini qisqartirmaydi — model elementini o'chirish opi.
- GIFT eksporti yozilgan, chiqishga ulanmagan (AUDIT-22).
- Keyingi dasturlar: AUDIT-21 (krossvord/flesh karta/infografika), AUDIT-22 (TTS — Azure uz-UZ / Aisha AI kalitlari egasidan; interaktiv runtime), AUDIT-23 (atestatsiya).

...
| P14c | P14 sharhi (CHANGES: C1 P1, C2–C6 P2, C7–C10 P3) → 3 parallel agent: ta'mir faqat kesilgan slaydda O'RNI bo'yicha (son qulf, kesilmagan band baytma-bayt, `qaytaring:` faqat kesilgan maydon), ustunlar har tomon o'z soni bilan, muqova sarlavhasi hech qachon so'ralmaydi, prompt ziddiyatsiz, rad sabablari logda; detektor zaxirasi — quti maqsadni chegaralasa chegara ⌊0.75 × quti⌋ (`DETECTOR_SHARE`, jonli 8–9 sinf circle: prompt «6–8», 7 so'zli band yupqa emas); sarlavha 4–6 so'z ≤ 61 belgi; `clippedAt` quyruq zaxirasi; slide-audit kesilgan maydonni bir marta hisoblaydi. 60/60 test, 8 + 4 + 1 mutatsiya qizil. |
| P14d | P14c re-review (CHANGES: N1 P2, N2 P2, N3–N8 P3) → 3 parallel agent: ta'mir promptidagi «son va tartib» qulfi FAQAT kesilgan-xolos slaydlarga (yupqa slayd yana chuqurlashtiriladi), almashtirilgan band uchun so'z poli, o'lik muqova filtri olib tashlandi; zaxira yaxlitlash (`Math.round`: 5 so'zli quti → 4, 8 → 6), zaxira faqat detektorli maydonlarda (iqtibos/bo'lim oralig'i eskicha), rejasiz reja nomi ≤ `TITLE_WORDS.max`; slide-audit «…» oldida raqam/qo'shtirnoq/qavs bo'lsa ham hisoblaydi. 69/69 + 35/35 + 46/46, 3 + 3 + 3 mutatsiya qizil. |

...
test("INT-02: 1–4-sinf/split/6 band — reja sarlavhasi 3 so'zga chegaralanadi (tor agenda quti)", () => {
  ...
  // Umumiy sarlavha qoidasi («4–6 so‘z, ≤ 61 belgi» — `TITLE_WORDS`/`TITLE_CHARS`, P14 C8: 6 × 9 ≤ 61) BOSHQA slaydlar uchun saqlanadi — bu qator uni almashtirmaydi.
  const p = promptWith({ slideAudience: "school_1_4", planItems: 6, blocks: "reja" }, compare);
  assert.match(p, /Sarlavha to‘liq fikr, 4–6 so‘z, ≤ 61 belgi\./);
  assert.match(p, /boshqa slaydlar sarlavhasi umumiy qoidada \(4–6 so‘z\) qoladi/);
});

...
const UZ: DocLabels = {
  ...
  subject: "Fan",
  // Idempotent: foydalanuvchi «Energetika fakulteti»/«301-guruh» deb yozsa qo'shimcha takrorlanmaydi
  // (AUDIT-19 ko'z tekshiruvi: «fakulteti fakulteti», «301-guruh-guruh»). `department` kabi.
  faculty: (n) => (/fakultet/i.test(n) ? n : `${n} fakulteti`),
  department: (n) => (n.toLowerCase().includes("kafedra") ? n : `${n} kafedrasi`),
  course: (n) => (/kurs/i.test(n) ? n : `${n}-kurs`),
  group: (n) => (/guruh/i.test(n) ? n : `${n}-guruh`),
...
const RU: DocLabels = {
  ...
  subject: "Предмет",
  faculty: (n) => (/факультет/i.test(n) ? n : `Факультет ${n}`),
  department: (n) => (n.toLowerCase().includes("кафедр") ? n : `Кафедра ${n}`),
  course: (n) => (/курс/i.test(n) ? n : `${n} курс`),
  group: (n) => (/групп/i.test(n) ? n : `группа ${n}`),
...
const EN: DocLabels = {
  ...
  subject: "Subject",
  faculty: (n) => (/faculty/i.test(n) ? n : `Faculty of ${n}`),
  department: (n) => (n.toLowerCase().includes("department") ? n : `Department of ${n}`),
  course: (n) => (/year|course/i.test(n) ? n : `Year ${n}`),
  group: (n) => (/group/i.test(n) ? n : `Group ${n}`),
...
const SLIDE_LABELS: Record<string, SlideLabels> = {
  ...
  kaa: {
    agenda: "Reje",
    conclusion: "Juwmaq",
    questions: "Sorawlar hám talqılaw",
    presentation: "Prezentaciya",
    quiz: "Baqlaw testi",
    references: "Paydalanılǵan ádebiyatlar",
    answers: "Test juwapları",
    goals: "Maqsetler",
  ...
  tg: {
    agenda: "Барномаи кор",
    conclusion: "Хулоса",
    questions: "Саволҳо ва муҳокима",
    presentation: "Презентатсия",
    quiz: "Санҷиши назоратӣ",
  ...
  tk: {
    agenda: "Meýilnama",
    conclusion: "Netije",
    questions: "Soraglar we pikir alyşma",
    presentation: "Prezentasiýa",
    quiz: "Barlag testi",
    references: "Peýdalanylan edebiýatlar",

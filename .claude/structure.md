...
- **Maqola 3 / AUDIT-18 WP-B — sxema turlari**: `figures/layout-{layers,cycle,timeline,matrix,compare}.ts` (+ mavjud flow/process/tree/prisma/chart — 10 tur), `model.ts` primitivlar (arc/dot/arrowStart/indent, `wrapToWidth`), `svg.ts` arrow-rev; `figureSpecFromLlm` tur tekshiruvi + `figureKinds` oq ro'yxati; prompt `FIGURE_KIND_HELP`/`FIGURE_KIND_HINTS` (tur→tavsiya); reyestr `figureKinds` (16-parametr); forma «Sxema turlari» chips; `scripts/figures-lab.mts`; testlar `figures-{layers,cycle,timeline,matrix,compare}`; live `article-kinds`.
- **Maqola 3 / AUDIT-18 WP-C — tur/profil ovozi**: `ArticleType.guidance` (tizim promptida «TYPE RULES»), `ArticleType.judge` (`describe`/`skip` — turga bog'liq baholovchi; `JudgeResult.skipped` ball/bandlardan chiqadi), `PublicationProfile.maxPages` → hisobot `pageLimit` (`plan.ts estimateDocPages`) + forma `data-pages-limit`; `tests/article-matrix` (12×5 qulf). **Q-2 halollik (R)**: `polish.ts needsUserData` faktlar bo'lsa ham (`unreported`), qayta yozish promptida `HONESTY_LIMIT`, baholovchi tavsiyalari chegarasi (`review.ts judgeSystemPrompt`), `criterionFixes` (past mezon → halol ko'rsatma). Mahalliy amaliyot tahlili — `docs/research/konf-2025-uzb.md`.

### O'qituvchi vositalari 2 + Test (AUDIT-20, 2026-09-16…17) — dars rejasi/xarita/glossariy/keys yangi dvigatel, Test yaratuvchi, o'quv bazasi

- **Dvigatel** `lib/generation/teacher/` — `types.ts` (5 kind modeli: lesson/map/glossary/keys/test, `TeacherModel = AcademicDoc.teacher`, `TEACHER_LIMITS`, variant 1/2/4), `registry.ts` (21 tur, `guidance`, `JudgeSpec`, `TEACHER_RULE_IDS`), `input.ts` (`teacherInputFromValues`/`encodeTeacherValues`), `prompts.ts` (TYPE RULES, `sourceBlock`, `curriculumBlock`; yorliqlar `i18n.ts teacherExtraLabels` dan), `engine.ts` (`buildTeacherDoc` → `lesson/map/glossary/keys.ts`, `complete` + `CostMeter`, `mapPool`), `guard.ts` (sof normalizatsiya: daqiqa yig'indisi, alifbo, rubrika, realism), `review.ts` (kind qoidalari + baholovchi), `polish.ts` (`runTeacherPolish`, glossariy MODEL shaklida qayta yoziladi, op lar bittalab), **`layout.ts planTeacher` — YAGONA MANBA** (`render-docx.ts drawTeacher`, `viewers/flow.ts teacherFlow`, `docx-profile.ts teacherProfile`, `title-model.ts` maktab shapkasi — «Tasdiqlayman», titul beti YO'Q), `edit.ts` (`TeacherOp`, `applyTeacherOps` — nasr↔model izchil, `table:<n>`, `teacher.<kind>.<…>` yo'llari, legacy 409), `legacy.ts` (eski hujjatlar faqat o'qish), `samples.ts`; `teacher/test/` (`questions.ts` normalizatsiya/seeded variantlar/kalit/ball, `omr.ts` + `figures/omr.ts` SVG→PNG, `prompts.ts`, `engine.ts buildTestDoc`, `review.ts` 20 qoida + `answerCorrectness`, `export.ts` GIFT); `teacher-params.ts` (38 param + zond — test kindida `testInputFromValues`).
- **O'quv bazasi** — statik `data/curriculum/` (8 fan, 42 fan×sinf, 2 424 mavzu — uzbmb.uz RTM PDF; `CURRICULUM-SOURCES.md`), `lib/curriculum.ts` (`curriculumIndex`/`curriculumTopics`/`hasCurriculum`/`pickTopics`, `FILES` jadvali qo'lda), `scripts/{fetch,gen}-curriculum.mts`, `app/api/curriculum` (60/min).
- **Forma** `components/forms/TeacherComposer.tsx` (`custom:"teacher"`, 5 vosita; test rejimlari mavzu/fayl/darslik — `CurriculumPicker`), `lib/tools.ts` `test` 3 000; 4 mavjud vosita `fields: []` + `custom:"teacher"`; `CUSTOM_REQUIRED.teacher` = university + author.
- **Server** — `edit-adapters.ts teacherAdapter`, `doc-polish.ts POLISHERS.teacher`, `article-rewrite.ts rewriteTeacher`, `assets.ts` (`doc.teacher.figures` OMR PNG), `preview.ts`; `components/files/useTeacherEdit.ts`; `ArticleEditor teacherEditTargets`; `index.ts` darvozalari element soni bilan (`teacherGateFail`), `delivered.ts` teacher shoxi, `structure.ts` `teacher:<id>`; `worker.ts` manba `tool.modes`.
- **O'chirildi**: `LessonViewer/TableViewer/GlossaryViewer/KeysViewer`, `viewerKind lesson|table|glossary|keys` → `teacher`. Eski yo'l `write-specials.ts` faqat `TEACHER_ENGINE=0`.
- Jonli: `npm run live -- lesson|map|map-quarters|glossary|keys|test-topic|test-file --source <docx>|test-curriculum`. Jurnal `docs/AUDIT-20.md`, tadqiqot `docs/research/*.md`.

### Talaba ishlari 2 (AUDIT-19, 2026-09-15…) — kurs ishi/referat/mustaqil ish, insho, tezis yangi avlod dvigatellari

- **`lib/generation/report/`** (R0-A) — neytral hisobot/sayqal qatlami: `types.ts` (`DocReview`, `ReviewCheck`, `PolishLog`, `UserNeed`, `JudgeResult<C>`, `JudgeSpec<C>`), `score.ts` (`scoreReviewFor`, `trigrams`/`jaccard`, `visualCoverageOf`, `check`/`rewrite`), `judge.ts` (`judgeSystemPromptFor`/`parseJudgeFor`/`neutralJudgeFor`/`judgeChecksFor`), `text.ts` (`sampleForJudge` — kirish/xulosa to'liq), `guard.ts` (`guardSection`, `wordsOf`, `factNumbers`), `filler.ts` (`FILLER_PHRASES`), `polish-core.ts` (`needsUserData`, `HONESTY_LIMIT`, `applyPolishWith`, `runPolishWith<Op,J>`, `acceptDelta`). `article/review.ts`/`polish.ts`/`guard.ts` — yupqa o'ramlar (xulq o'zgarmagan).
- **Tezis** (WP-F) — `lib/tools.ts` thesis `custom:"article"`, `THESIS_TYPE_IDS`/`THESIS_PRICES`/`thesisTypeId`; `write-llm.ts` thesis → `buildArticleDoc` (IMRAD yo'li o'chirildi); `structure.ts`/`index.ts isArticleTool`/`docx-profile.ts`/`lib/viewers/kind.ts` thesis shoxlari; `ArticleComposer` tezis rejimi (`allowed` turlar), `ArticleTypeGallery allowed/title`; eski tezis hujjatlari — GOST titul bilan avvalgidek.
- **Talaba ishi dvigateli** `lib/generation/work/` (WP-A, WP-C) — `types.ts` (3 janr × 8 tur, `WorkModel = AcademicDoc.work`), `registry.ts` (skelet, kirish elementlari, `refsMin`, `guidance`, `JudgeSpec`), `subjects.ts` (5 fan profili), `input.ts` (`workInputFromValues`/`encodeWorkValues`), `plan.ts` (`workWordPlan` apparatura ayirilgan, `workVisualNumbers` — referatda tekis), `prompts.ts` (PRIMARY/OTHER manba taqsimoti, `workExpandPrompt`), `engine.ts` (`buildWorkDoc`: research → outline → intro → paragraflar `mapPool(3)` + `assignPrimaryRefs` + kengaytirish `WORK_EXPAND_BELOW 0,85` → vizuallar → xulosa → refs → hisobot → sayqal), `guard.ts`, `review.ts` (16 qoida + baholovchi), `polish.ts` (`runWorkPolish`, `apply: applyWorkOps`), **`layout.ts planWork` — YAGONA MANBA** (`render-docx.ts drawWork`, `lib/viewers/flow.ts workFlow`, `docx-profile.ts workProfile`, `title-model.ts` talaba ishi tituli), `edit.ts` (`WorkOp`, `applyWorkOps`/`inverseWorkOps`/`parseWorkOps`), `samples.ts`; `lib/server/edit-adapters.ts workAdapter`; `components/files/useWorkEdit.ts`; `work-params.ts` (27 parametr + zond).
- **Insho dvigateli** `lib/generation/essay/` (WP-D/E1) — kontekst × tur reyestri, `rubric.ts` vaznlar bitta joyda, `buildEssayDoc`, `reviewEssay`, `runEssayPolish`; `EssayComposer`; `essay-params.ts`.
- **Manbalar** (WP-B) — `research/googlebooks.ts` (`searchBooks`/`verifyIsbn`, kesh `gb:`), `research/isbn.ts` (SOF `normalizeIsbn` — klient bandli uchun), `research/lexuz.ts` (`findLaws` → `fetchLaw` tasdiq, kesh `lex:`), `cite/order.ts orderUzReferences`, `formatGostBook/Law/Web`.
- **Formalar** — `components/forms/WorkComposer.tsx` (`custom:"work"`, `CUSTOM_REQUIRED.work`), `EssayComposer.tsx`, `ArticleComposer` tezis rejimi; `lib/server/doc-polish.ts POLISHERS` (article|essay|work); `index.ts`: hajm darvozasi talaba ishida `workGateWords` × `bodyWordCount` (faqat matn).
- **Klient chegarasi** — `tests/client-boundary.test.mts`: `lib/tools.ts` va `*Composer.tsx` tranzitiv importlari (dinamik ham) `lib/server/**`/`server-only` ga yetmasligi kerak (AUDIT-19 da `article/input.ts → googlebooks → cache → db` zanjiri sahifalarni 500 qilgan edi).
- Jonli: `npm run live -- referat|coursework-theory|coursework-applied|independent|essay-dtm|essay-academic|essay-ielts|thesis` (talaba ishi holatlari `doc.json` yozadi). Jurnal `docs/AUDIT-19.md`.

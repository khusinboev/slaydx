...
PG_PROFILE="${PG_PROFILE:-auto}"      # auto | tuned | default
...
pg_args() {
  ...
  if [[ "$profile" == tuned ]]; then
    ...
    echo "--memory 1g --cpus 1 --shm-size 256m $PG_IMAGE postgres -c max_connections=100 -c shared_buffers=256MB -c work_mem=16MB -c shared_preload_libraries=pg_stat_statements -c log_min_duration_statement=500 -c autovacuum_vacuum_scale_factor=0.05 -c idle_in_transaction_session_timeout=60s"
  else
